...
function generateTestFor({ entry, hash, ext, ratio }) {
  return async () => {
    ...
    const [, normalSrc, webpSrc, lqipSrc, sizesSrc, allSrc] = stats.modules.map(z => z.source)
    ...
    expect(getJsonSrc(webpSrc).webp).toEqual(`img/${hash}.${ext}.webp`)
    expect(getJsonSrc(lqipSrc).lqip).toMatch(/^data:image\/jpeg;base64,/)

    const sizes = getJsonSrc(sizesSrc).sizes
    expect(sizes).toHaveProperty('200w', `img/${hash}.200w.${ext}`)
    expect(sizes).toHaveProperty('800w', `img/${hash}.800w.${ext}`)
